...
def setup_paths():
    ...
    temp_dir = os.path.join(base_dir, 'Files')
    graphics_dir = os.path.join(base_dir, 'Graphics')
    os.makedirs(temp_dir, exist_ok=True)
    os.makedirs(graphics_dir, exist_ok=True)
    return temp_dir, graphics_dir

TempDirPath, GraphicsDirPath = setup_paths()
...
class ChatSection(QWidget):
    ...
    def toggle_icon(self, event=None):
        try:
            if self.toggled:
                self.load_icon(os.path.join(GraphicsDirPath, 'voice.png'), 60, 60)
                MicButtonInitialed()
            else:
                self.load_icon(os.path.join(GraphicsDirPath, 'mic.png'), 60, 60)
                MicButtonClosed()
...
class InitialScreen(QWidget):
    ...
    def  __init__(self, parent=None):
        try:
            ...
            # Setup GIF display
            gif_label = QLabel()
            movie_path = os.path.join(GraphicsDirPath, 'Jarvis.gif')
